...
'''
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))#这里的accuracy是什么？相当于是起了一个新名字
...
print(tf.reduce_mean(tf.cast(correct_prediction, tf.float32)).eval({x: mnist.test.images, y_:mnist.test.labels}))
