# -*- coding: utf-8 -*-
"""
Created on Sun Mar 11 16:17:58 2018
...
def subsample(inputs, factor, scope = None):
    '''
    :param inputs:
    :param factor: 如果为1，则不做修改直接返回inputs，如果不为1，则使用最大池化进行降采样
    :param scope:
    :return:
    '''
    if factor == 1:
...
def conv2d_same(inputs, num_outputs, kernel_size, stride, scope = None):
    if stride == 1:#如果为1，直接用padding = 'SAME'
...
def stack_blocks_dense(net, blocks, outputs_collections = None):
    ...
    :param ouputs_collections: 收集各个end_points的collections
    :return:
    '''
    for block in blocks:
...
def resnet_arg_scope(is_training = True, weight_decay = 0.0001, batch_norm_decay = 0.997,
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        with slim.arg_scope([slim.batch_norm], **batch_norm_params):
            with slim.arg_scope([slim.max_pool2d], padding = 'SAME') as arg_sc:#原文这里是VALID
                return arg_sc
...
def bottleneck(inputs, depth, depth_bottleneck, stride, outputs_collections = None, scope = None):
    '''
    :param inputs:
    :param depth: Blocks class args
    :param depth_bottleneck: Block class args
    :param stride: Block class args
    :param outputs_collections: 收集end_points
    :param scope:
    :return:
    '''
...
################################################################################
#resnet_v2
#ResNET V2的主函数，预先定义好网络的残差学习模块组blocks，就可以生成对应的完整的ResNet
################################################################################
def resnet_v2(inputs, blocks, num_classes = None, global_pool = True,
    ...
    :param num_classes: num_class是最后输出的类数
    :param global_pool: 标志是否加上最后一层全局平均池化
    :param include_root_block: 标志是否加上ResNet网络最前面通常使用的7X7卷积核最大池化
    :param reuse: 标志是否重用
    :param socpe:
    :return:
    '''
...
with slim.arg_scope(resnet_arg_scope(is_training = False)):
    net, end_points = resnet_v2_152(inputs, 100)
